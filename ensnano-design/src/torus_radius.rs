...
fn main() {
    let curve = CurveDescriptor2D::Ellipse { semi_minor_axis: 1f64.into(), semi_major_axis: 1.5f64.into() };
    ...
    while !found {
        ...
        let desc = TwistedTorusDescriptor {
            ...
            curve: curve.clone(),
            helix_index_shift_per_turn: -4,
            number_of_helix_per_section: 20,
        ...
        let curve = InstanciatedCurveDescriptor::try_instanciate(Arc::new(CurveDescriptor::TwistedTorus(desc))).unwrap().make_curve(&Parameters::GEARY_2014_DNA, &mut cache);
